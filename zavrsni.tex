...
\begin{figure}[!h]
    ...
    \label{img:repol}
\end{figure}
\emph{Podržano učenje} grana je \emph{strojnog učenja} \engl{machine learning} zajedno sa \emph{nadziranim} \engl{supervised} i \emph{nenadziranim učenjem} \engl{unsupervised learning}, kao što je prikazano na slici \ref{img:repol}.
...
Dva glavna pristupa u implementaciji sustava LCS su \emph{michiganski stil} \engl{Michigan-Style} i \emph{pittsburgski stil} \engl{Pittsburgh-Style}.
...
S obzirom na složenost originalnog sustava LCS, malo jednostavniju i razumljiviju inačicu dao je Stewart W. Wilson pod nazivom ZCS \engl{"zeroth-level" classifier system}.
...
Redom se ažuriraju iskustvo pravila \engl{experience} $exp$, njegovo predviđanje \engl{prediction} $p$, pogreška u predviđanju \engl{prediction error} $\epsilon$, preciznost \engl{accuracy} $\kappa$, relativna preciznost \engl{relative accuracy} $\kappa'$, dobrota $F$ i prosječna veličina akcijskih skupova koji su sadržavali to pravilo \engl{action set size} $as$.
...
\kljucnerijeci{Sustavi LCS, strojno učenje, podržano učenje, genetski algoritmi, ponovno iskorištavanje znanja.}
\end{sazetak}
